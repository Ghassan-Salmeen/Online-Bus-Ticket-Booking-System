...
if (isset($_POST['search'])) {
    ...
    //// If no buses are found with vacancies, display a message or search for all buses between the locations
...
//Generates a booking form for users to insert passenger name and number of passengers
if (isset($_POST['book'])) {
	$jid=$_POST['jid'];
	$bid=$_POST['bid'];
	$seat=$_POST['seat'];
...
if (isset($_POST['booking'])) {
	...
	$uid=$_SESSION['uid'];
	$date=date("Y-m-d");
  //to calculate vacant seats
	$vacant=$seat-$no_p;

  //// Check if there are existing bookings for the same bus on the same date
    ...
    if (mysqli_num_rows($run)>0) {
      ...
      if ($run) {
          ...
          //// Check if ticket already exists for the user on the same bus and date
...
//to display the tickets to user
...
//to search buses by id and display their booking details
if (isset($_POST['bkd'])) {
  $bid=$_POST['bid'];
   $sql2="SELECT * FROM bus_details WHERE bus_id='$bid'";
          $run2=mysqli_query($con,$sql2);
...
//to show all tickets to the admin with the given bus id and date
if (isset($_POST['tbkd'])) {
   $bid=$_POST['bid'];
   $pdate=$_POST['date'];
   $sql2="SELECT * FROM bus_details WHERE bus_id='$bid'";
